...
collect_fallback_failure_bundle() {
    ...
    collect_command="${REMOTE_SSH_COMMAND} \"cd ${REMOTE_DIR} && chmod +x vd_io/collect_failure_bundle.sh vd_io/enable_failure_coredumps.sh && timeout --kill-after=${failure_bundle_kill_after_seconds}s ${failure_bundle_collect_timeout_seconds}s env NODE_IP=${NODE_IP} REMOTE_DIR=${REMOTE_DIR} RUN_KEY=remote_runner BUNDLE_REASON=remote_test_rc_${test_rc} bash -c 'vd_io/enable_failure_coredumps.sh && vd_io/collect_failure_bundle.sh'\""
...
collect_io_signature() {
    timeout --kill-after=5s 20s bash -c "
        ${REMOTE_SSH_COMMAND} \"cd ${REMOTE_DIR} && chmod +x vd_io/io_progress_signature.sh && vd_io/io_progress_signature.sh\"
...
# Re-points KDUMP_COREDIR to this build REMOTE_DIR so older CI paths cannot steal panics.
eval "${REMOTE_SSH_COMMAND} \"cd ${REMOTE_DIR} && chmod +x vd_io/enable_failure_coredumps.sh vd_io/enable_failure_kdump.sh vd_io/enable_draid_pending_debug.sh && NODE_IP=${NODE_IP} REMOTE_DIR=${REMOTE_DIR} ENABLE_COREDUMPS=1 vd_io/enable_failure_coredumps.sh && NODE_IP=${NODE_IP} REMOTE_DIR=${REMOTE_DIR} ENABLE_KDUMP=1 vd_io/enable_failure_kdump.sh && NODE_IP=${NODE_IP} REMOTE_DIR=${REMOTE_DIR} vd_io/enable_draid_pending_debug.sh\"" || true
...
salvage_command="${REMOTE_SSH_COMMAND} \"cd ${REMOTE_DIR} && python3 vd_io/salvage_junit_reports.py --stop-monitor --output node-report.xml\""
...
if [ -d "${tmp_results}" ]; then
    run_cleanup_command "mark Allure target context" "${report_command_timeout_seconds}" \
        python3 vd_io/mark_allure_target_context.py "${tmp_results}" "${NODE_IP}" || true
...
if [ ! -f "${report_file}" ]; then
    ...
    run_cleanup_command "merge per-item JUnit reports" "${report_command_timeout_seconds}" \
        python3 vd_io/salvage_junit_reports.py --from-dir "${item_dir}" --output "${report_file}" || true
    rm -rf "${item_dir}"
fi
