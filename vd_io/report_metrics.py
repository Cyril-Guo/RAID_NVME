...
def test_execution_infra_stats():
    """Infra hard-fails that should affect case pass/fail without raising Total.

    Only Test_Execution (allure or test_execution_*.log). Restore/env prepare do
    not inflate selected-case counters.
    """
    stats = empty_stats()
    ...
    # Fallback: status logs for execution only (not environment_prepare_*).
    manually_aborted = console_was_manually_aborted()
    for path in sorted(glob.glob("test_execution_*.log")):
        text = _read_text(path)
        if manually_aborted and not execution_log_has_explicit_failure(text):
...
def report_metrics():
    ...
    # JUnit green but Test_Execution hard-fail: fold into errors/failures without
    # inflating Total beyond selected/executed case count.
    ...
    # Feishu Total = checked selection count; pass/fail from real execution only.
