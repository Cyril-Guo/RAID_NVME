#!/usr/bin/env bash
# Enable userspace coredumps on the DUT for VD_IO triage:
...
ENABLE_COREDUMPS=${ENABLE_COREDUMPS:-1}
...
if [ -d /etc/security/limits.d ] && [ -w /etc/security/limits.d ]; then
    cat >"${LIMITS_FILE}" <<'EOF'
# Managed by RAID_NVME vd_io/enable_failure_coredumps.sh — unlimited userspace cores.
*       soft    core    unlimited
*       hard    core    unlimited
...
EOF
    ...
    if ! grep -q 'RAID_NVME vd_io/enable_failure_coredumps' /etc/security/limits.conf 2>/dev/null; then
        cat >>/etc/security/limits.conf <<'EOF'

# Managed by RAID_NVME vd_io/enable_failure_coredumps.sh
*       soft    core    unlimited
*       hard    core    unlimited
root    soft    core    unlimited
root    hard    core    unlimited
