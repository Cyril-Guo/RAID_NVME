...
def main():
    ...
    if total <= 0 and not failure_summary:
        # CLI may still notify when execution was attempted / console exists.
        if env("ALLOW_EMPTY_WITH_ATTEMPT", "") == "1" and (
            env("TEST_EXECUTION_ATTEMPTED", "") == "true"
            or os.path.exists("jenkins_console.log")
        ):
            total = 0
        else:
    ...
    # No selected cases but attempted: keep blue/red from build result only.
    if total == 0:
    ...
    if mr_iid:
        ...
        driver_lines.append(
            f"Branch: {env('KERNEL_DRIVER_REF', env('KERNEL_DRIVER_BRANCH', 'main'))}"
        )
    driver_lines.append(f"Commit: {env('KERNEL_DRIVER_COMMIT', 'unknown')}")
    driver_lines.append(
        f"raid_cli({env('RAID_CLI_BRANCH', 'hostraid_cli')}): {env('RAID_CLI_COMMIT', 'unknown')}"
    )
    ...
    if total > 0:
        ...
        stats_text = "本次无计数字段（未选择用例或无 junit）；详情请打开 Allure。"
