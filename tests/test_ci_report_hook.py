...
def test_context_hook_and_real_fixture_evidence(tmp_path, monkeypatch):
    ...
    completed = subprocess.run([sys.executable, "-m", "pytest", "-q", "test_dummy.py",
                               "--alluredir=allure-results", "--junitxml=case-report_test_ci_03_mix_4k__2.xml"],
    ...
    nvme_raid_test.merge_junit_reports(["test_ci_03_mix_4k__2"], "node-report_192.168.22.134.xml")
