...
def test_case_logs_survive_missing_pytest_finalization(tmp_path):
    ...
    case = tmp_path / "cases" / "test_ci_05_mix_4k__2"
    logs = case / "IO_Stress" / "log" / "ResultLog"
    logs.mkdir(parents=True)
    (logs / "result.log").write_text("FIO stage failed\n")
    (case / "fio_command_output_test_ci_05_mix_4k__2.log").write_text("watchdog timeout\n")
    (case / "report_test_ci_05_mix_4k__2.xml").write_text('<testsuite><testcase name="test_mix_stress" /></testsuite>')
    recover_case_outputs(tmp_path)
    root = tmp_path / "allure-results"
    index = json.loads((root / "case_test_ci_05_mix_4k__2_monitor_attachments.json").read_text(encoding="utf-8"))
    assert index[0]["run_key"] == "test_ci_05_mix_4k__2"
    with tarfile.open(root / index[0]["attachment"]["source"]) as archive:
        assert "IO_Stress/log/ResultLog/result.log" in archive.getnames()
        assert "fio_command_output_test_ci_05_mix_4k__2.log" in archive.getnames()
    assert (tmp_path / "report_test_ci_05_mix_4k__2.xml").exists()


def test_case_sidecars_do_not_overwrite_each_other(tmp_path):
    from ci.case_artifacts import recover_case_outputs
    for key in ("test_ci_05_mix_4k__2", "test_ci_05_mix_4k__5"):
        root = tmp_path / "cases" / key / "allure-results"
        root.mkdir(parents=True)
        (root / "monitor_attachments.json").write_text(json.dumps([
            {"item": "test_ci_05_mix_4k", "attachment": {"name": key, "source": f"monitor_{key}.tar.gz"}}]))
...
def test_downloaded_fallback_bundle_attaches_to_matching_case(tmp_path, monkeypatch):
    ...
    root = tmp_path / "allure-results"
    root.mkdir()
    for key in ("test_ci_05_mix_4k__2", "test_ci_07_random_io_4k__5"):
    ...
    (tmp_path / "test_execution_192.168.22.134.log").write_text("[ITEM_START] test_ci_05_mix_4k__2\n[ITEM_END] test_ci_05_mix_4k__2 exit_code=1\n")
    ...
    for key in ("test_ci_05_mix_4k__2", "test_ci_07_random_io_4k__5"):
        ...
        assert bool(archives) == (key == "test_ci_05_mix_4k__2")
...
def test_case_archive_is_bounded_and_records_truncation(tmp_path, monkeypatch):
    ...
    with tarfile.open(tmp_path / "allure-results" / "case_debug_test_ci_05_mix_4k__2.tar.gz") as archive:
...
def test_recovery_repairs_partial_json_without_overwriting_completed_results(tmp_path):
    from ci.case_artifacts import recover_case_outputs
    source = tmp_path / "cases" / "test_ci_05_mix_4k__2" / "allure-results"
...
def test_collection_after_success_has_no_false_case_attribution(tmp_path):
    ...
    log.write_text("[ITEM_START] test_ci_05_mix_4k__2\n[ITEM_END] test_ci_05_mix_4k__2 exit_code=0\n")
    assert last_run(log) == ""
    log.write_text("[ITEM_START] test_ci_05_mix_4k__2\n[ITEM_END] test_ci_05_mix_4k__2 exit_code=0\n[ITEM_START] test_ci_07_random_io_4k__5\n")
    assert last_run(log) == "test_ci_07_random_io_4k__5"
...
def test_interrupted_case_logs_attach_to_execution_failure_without_junit(tmp_path, monkeypatch):
    ...
    (tmp_path / "test_execution_192.168.22.134.log").write_text(
        "[ITEM_START] test_ci_05_mix_4k__2\nidle watchdog fired\nTEST_EXECUTION_STATUS=failed\n")
    junit_to_allure.main()
    ...
    logs = next(s for s in result["steps"] if s["name"] == "日志收集")["attachments"]
    assert any(a["source"].endswith("case_debug_test_ci_05_mix_4k__2.tar.gz") for a in logs)
