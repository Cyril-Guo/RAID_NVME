...
from vd_io import mark_allure_target_context
        ...
        json.dumps(
            {
                ...
                "labels": [{"name": "suite", "value": "test_vd_io_03_mix_4k"}],
                "attachments": [
                    {
...
def test_mark_allure_target_context_marks_physical_history_and_attachments(tmp_path):
    ...
    allure_dir.mkdir()
    write_result(allure_dir / "physical-result.json", "FIO test: lawdiskstress", "same-history", "monitor_log_test_vd_io_03_mix_4k.tar.gz")
    (allure_dir / "monitor_log_test_vd_io_03_mix_4k.tar.gz").write_bytes(b"physical")
    ...
    assert physical["attachments"][0]["source"] == "physical_192_168_22_134_monitor_log_test_vd_io_03_mix_4k.tar.gz"
...
def test_mark_allure_target_context_marks_pending_monitor_sidecar(tmp_path):
    ...
    allure_dir.mkdir()
    (allure_dir / "monitor_log_test_vd_io_03_mix_4k.tar.gz").write_bytes(b"log")
            ...
            [
                {
                    ...
                    "attachment": {
                        "name": "monitor_log_test_vd_io_03_mix_4k",
                        "source": "monitor_log_test_vd_io_03_mix_4k.tar.gz",
                        "type": "application/gzip",
                    },
                }
            ]
    ...
    assert pending[0]["item"] == "test_vd_io_03_mix_4k"
    assert pending[0]["host"] == "192.168.22.134"
    assert pending[0]["target"] == "physical"
    assert pending[0]["attachment"]["source"] == "physical_192_168_22_134_monitor_log_test_vd_io_03_mix_4k.tar.gz"
