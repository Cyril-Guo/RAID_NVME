...
def test_case_logs_survive_missing_pytest_finalization(tmp_path):
    ...
    (case / "fio_command_output_test_powercycle_02_dc__2.log").write_text("watchdog timeout\n")
    (case / "report_test_powercycle_02_dc__2.xml").write_text('<testsuite><testcase name="test_dc_powercycle" /></testsuite>')
    recover_case_outputs(tmp_path)
    root = tmp_path / "allure-results"
    index = json.loads((root / "case_test_powercycle_02_dc__2_case_attachments.json").read_text(encoding="utf-8"))
    assert index[0]["run_key"] == "test_powercycle_02_dc__2"
    with tarfile.open(root / index[0]["attachment"]["source"]) as archive:
        assert "IO_Stress/log/ResultLog/result.log" in archive.getnames()
        assert "fio_command_output_test_powercycle_02_dc__2.log" in archive.getnames()
    assert (tmp_path / "report_test_powercycle_02_dc__2.xml").exists()
...
def test_case_sidecars_do_not_overwrite_each_other(tmp_path):
    ...
    for key in ("test_powercycle_02_dc__2", "mix__5"):
        ...
        (root / "case_attachments.json").write_text(json.dumps([
            {"item": "test_powercycle_02_dc", "attachment": {"name": key, "source": f"monitor_{key}.tar.gz"}}]))
        (root / f"monitor_{key}.tar.gz").write_bytes(b"monitor")
    recover_case_outputs(tmp_path)
    root = tmp_path / "allure-results"
    for key in ("test_powercycle_02_dc__2", "mix__5"):
        entries = json.loads((root / f"{key}_case_attachments.json").read_text())
...
def test_downloaded_fallback_bundle_attaches_to_matching_case(tmp_path, monkeypatch):
    ...
    root = tmp_path / "allure-results"
    root.mkdir()
    for key in ("test_powercycle_02_dc__2", "random_io__5"):
    ...
    (tmp_path / "test_execution_192.168.22.134.log").write_text("[ITEM_START] test_powercycle_02_dc__2\n[ITEM_END] test_powercycle_02_dc__2 exit_code=1\n")
    ...
    for key in ("test_powercycle_02_dc__2", "random_io__5"):
        ...
        assert bool(archives) == (key == "test_powercycle_02_dc__2")
...
def test_case_archive_is_bounded_and_records_truncation(tmp_path, monkeypatch):
    ...
    with tarfile.open(tmp_path / "allure-results" / "case_debug_test_powercycle_02_dc__2.tar.gz") as archive:
...
def test_interrupted_case_logs_attach_to_execution_failure_without_junit(tmp_path, monkeypatch):
    ...
    (tmp_path / "test_execution_192.168.22.134.log").write_text(
        "[ITEM_START] test_powercycle_02_dc__2\nidle watchdog fired\nTEST_EXECUTION_STATUS=failed\n")
    junit_to_allure.main()
    ...
    logs = next(s for s in result["steps"] if s["name"] == "日志收集")["attachments"]
    assert any(a["source"].endswith("case_debug_test_powercycle_02_dc__2.tar.gz") for a in logs)
