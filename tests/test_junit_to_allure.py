...
from vd_io import junit_to_allure
...
<testsuite name="pytest" tests="1">
  <testcase classname="test_items.test_vd_io_01_lawdisk" name="test_lawdiskstress" />
</testsuite>
""",
        encoding="utf-8",
    )
...
def test_junit_to_allure_skips_node_junit_when_pytest_allure_exists(tmp_path, monkeypatch):
        ...
        json.dumps(
            {
                ...
                "labels": [
                    ...
                    {"name": "run_key", "value": "test_vd_io_01_lawdisk__2"},
                ],
            }
        ),
        encoding="utf-8",
    ...
    (tmp_path / "node-report_192.168.22.134.xml").write_text(
        """<?xml version="1.0" encoding="utf-8"?>
<testsuite name="pytest" tests="1">
  <testcase classname="test_items.test_vd_io_01_lawdisk__2" name="test_lawdiskstress" time="12.5" />
</testsuite>
""",
        encoding="utf-8",
    )
...
""",
    ...
    (tmp_path / "jenkins_console.log").write_text(
        "[Pipeline] stage\n"
        "[ITEM_START] test_vd_io_03_mix_4k\n"
        "[ITEM] test_vd_io_03_mix_4k -> test_items/test_vd_io_03_mix_4k.py\n"
        "all console output\n"
        "[ITEM_END] test_vd_io_03_mix_4k exit_code=0\n"
        "[Pipeline] echo\n",
        encoding="utf-8",
    )
...
def test_junit_to_allure_does_not_copy_global_fio_summary_onto_every_case(tmp_path, monkeypatch):
    ...
    (tmp_path / "node-report_192.168.23.94.xml").write_text(
        """<testsuites>
  <testsuite name="pytest">
    <testcase classname="test_items.test_vd_io_03_mix_4k" name="test_mix_stress" />
    <testcase classname="test_items.test_vd_io_04_mix_512" name="test_mix_stress" />
  </testsuite>
</testsuites>
""",
        encoding="utf-8",
...
def test_junit_to_allure_relabels_existing_pytest_terminal_as_debug_log(tmp_path, monkeypatch):
    ...
    result = {
        ...
        "historyId": "192.168.23.94::physical::test_items.test_vd_io_03_mix_4k::test_mix_stress",
        "fullName": "physical:192.168.23.94:test_items.test_vd_io_03_mix_4k#test_mix_stress",
        "testCaseId": "192.168.23.94::physical::test_items.test_vd_io_03_mix_4k::test_mix_stress",
        "labels": [
            {"name": "package", "value": "test_items.test_vd_io_03_mix_4k"},
            {"name": "framework", "value": "pytest"},
        ],
...
def test_junit_to_allure_skips_per_item_junit_reports(tmp_path, monkeypatch):
    ...
    allure_dir = tmp_path / "allure-results"
    allure_dir.mkdir()
    (tmp_path / "case-report_test_vd_io_01_lawdisk.xml").write_text(
        """<testsuite name="pytest">
  <testcase classname="test_items.test_vd_io_01_lawdisk" name="test_lawdiskstress" />
</testsuite>
""",
...
"""
    ...
    results = [json.loads(path.read_text(encoding="utf-8")) for path in allure_dir.glob("*-result.json")]
    test_results = [result for result in results if "test_mix_stress" in result["name"]]
    assert len(test_results) == 1
    assert {
        label["value"] for label in test_results[0]["labels"] if label["name"] == "target"
    } == {"physical"}
    assert test_results[0]["name"] == "[Physical 192.168.22.134] test_mix_stress"

    assert junit_to_allure.main() == 0
    rerun_results = [json.loads(path.read_text(encoding="utf-8")) for path in allure_dir.glob("*-result.json")]
    test_results = [result for result in rerun_results if "test_mix_stress" in result["name"]]
    assert len(test_results) == 1
...
def test_junit_to_allure_attaches_pending_monitor_only_to_matching_target(tmp_path, monkeypatch):
    ...
    other_node_result = {
        ...
        "historyId": "physical:192.168.22.125:test_mix_stress",
        "fullName": "physical:192.168.22.125:test_mix_stress",
        "testCaseId": "physical:192.168.22.125:test_mix_stress",
        "labels": [
    ...
    physical_result = {
        ...
        "historyId": "physical:192.168.22.134:test_mix_stress",
        "fullName": "physical:192.168.22.134:test_mix_stress",
        "testCaseId": "physical:192.168.22.134:test_mix_stress",
        "labels": [
            {"name": "host", "value": "192.168.22.134"},
            {"name": "target", "value": "physical"},
        ],
    ...
    (allure_dir / "monitor_attachments.json").write_text(
        json.dumps(
            [
                {
                    "item": "test_vd_io_03_mix_4k",
                    "host": "192.168.22.134",
                    "target": "physical",
                    "attachment": {
                        "name": "monitor_log_test_vd_io_03_mix_4k",
                        "source": "physical_monitor_log_test_vd_io_03_mix_4k.tar.gz",
                        "type": "application/gzip",
                    },
                }
            ]
    ...
    assert physical["attachments"][0]["source"] == "physical_monitor_log_test_vd_io_03_mix_4k.tar.gz"
