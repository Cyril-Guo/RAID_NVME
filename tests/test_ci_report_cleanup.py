...
def result(run="test_powercycle_02_dc__2", node="192.168.22.134", status="failed"):
    return {
        "uuid": f"{node}-{run}", "name": "FIO mix", "status": status,
        "fullName": f"physical:{node}:{run}::test_items.test_powercycle_02_dc#test_dc_powercycle",
...
def test_pytest_duplicates_removed_but_real_internal_error_preserved(tmp_path, monkeypatch):
    ...
    duplicate = result()
    duplicate.update(uuid="converted", fullName="physical:192.168.22.134:test_items.test_powercycle_02_dc__2#test_dc_powercycle")
...
def test_native_case_does_not_suppress_another_junit_case(tmp_path, monkeypatch):
    ...
    (tmp_path / "report_192.168.22.134.xml").write_text(
        '<testsuite name="pytest"><testcase classname="test_items.test_powercycle_02_dc__2" name="test_dc_powercycle">'
...
def test_sidecars_match_exact_host_and_run_key(tmp_path, monkeypatch):
    ...
    root = tmp_path / "allure-results"
    root.mkdir()
    for run in ("test_powercycle_02_dc__2", "mix__5"):
    ...
    write_json(root / "physical_134_case_attachments.json", [{"item": "test_powercycle_02_dc", "run_key": "test_powercycle_02_dc__2",
    ...
    for path in root.glob("*-result.json"):
        ...
        assert found == (path.name == "192.168.22.134-test_powercycle_02_dc__2-result.json")
...
def test_two_nodes_with_same_case_keep_separate_log_content(tmp_path, monkeypatch):
    ...
    for node in ("192.168.22.134", "192.168.22.135"):
        ...
        case = workspace / "cases" / "test_powercycle_02_dc__2"
