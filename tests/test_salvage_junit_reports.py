...
def test_merge_from_directory_writes_node_report(tmp_path):
    ...
    item_dir.mkdir()
    (item_dir / "report_test_ci_01_lawdisk_4k.xml").write_text(
...
""",
    ...
    assert items == ["test_ci_01_lawdisk_4k"]
...
def test_merge_from_directory_keeps_duplicate_run_keys(tmp_path):
    item_dir = tmp_path / "items"
    item_dir.mkdir()
    for run_key in ("test_ci_01_lawdisk_4k__2", "test_ci_01_lawdisk_4k__5"):
...
""",
    ...
    assert items == ["test_ci_01_lawdisk_4k__2", "test_ci_01_lawdisk_4k__5"]
    merged = output.read_text(encoding="utf-8")
    assert merged.count("testcase") == 2
    assert "test_items.test_ci_01_lawdisk_4k__2" in merged
    assert "test_items.test_ci_01_lawdisk_4k__5" in merged
