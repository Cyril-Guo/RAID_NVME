...
def result(run="test_vd_io_03_mix_4k__2", node="192.168.22.134", status="failed"):
    return {
        "uuid": f"{node}-{run}", "name": "FIO mix", "status": status,
        "fullName": f"physical:{node}:{run}::test_items.test_vd_io_03_mix_4k#test_mix_stress",
...
def test_pytest_duplicates_removed_but_real_internal_error_preserved(tmp_path, monkeypatch):
    ...
    duplicate = result()
    duplicate.update(uuid="converted", fullName="physical:192.168.22.134:test_items.test_vd_io_03_mix_4k__2#test_mix_stress")
...
def test_native_case_does_not_suppress_another_junit_case(tmp_path, monkeypatch):
    ...
    (tmp_path / "node-report_192.168.22.134.xml").write_text(
        '<testsuite name="pytest"><testcase classname="test_items.test_vd_io_03_mix_4k__2" name="test_mix_stress">'
        '<failure message="fio timeout" /></testcase>'
        '<testcase classname="test_items.test_vd_io_05_random_io_4k__5" name="test_random_io" /></testsuite>')
    junit_to_allure.main()
    results = [json.loads(p.read_text(encoding="utf-8")) for p in root.glob("*-result.json")]
    assert len(results) == 2
    assert any("test_vd_io_05_random_io_4k" in r["name"] for r in results)
...
def test_sidecars_match_exact_host_and_run_key(tmp_path, monkeypatch):
    ...
    root = tmp_path / "allure-results"
    root.mkdir()
    for run in ("test_vd_io_03_mix_4k__2", "test_vd_io_03_mix_4k__5"):
    ...
    write_json(root / "physical_134_monitor_attachments.json", [{"item": "test_vd_io_03_mix_4k", "run_key": "test_vd_io_03_mix_4k__2",
    ...
    for path in root.glob("*-result.json"):
        ...
        assert found == (path.name == "192.168.22.134-test_vd_io_03_mix_4k__2-result.json")
...
def test_two_nodes_with_same_case_keep_separate_log_content(tmp_path, monkeypatch):
    ...
    for node in ("192.168.22.134", "192.168.22.135"):
        ...
        case = workspace / "cases" / "test_vd_io_03_mix_4k__2"
