...
def test_mix_4k_stress():
    ...
    os.environ["MIX_RANDOM_CHOICE"] = "random_choice_4k.py"
    ...
    allure.dynamic.description(
        f"混合读写；4 路 MixIO 由 random_choice_4k.py 直接生成"
        f"（1000 组 × 30s，块大小 4k 对齐）。"
        f" MIX_FAIL_ON_ANY={fail_on_any or 'yes'}。"
    )
    print(
        f"[MIX] item={item} mode=mixstress MIX_RANDOM_CHOICE=random_choice_4k.py "
        f"MIX_FAIL_ON_ANY={fail_on_any or 'yes'}"
    )
