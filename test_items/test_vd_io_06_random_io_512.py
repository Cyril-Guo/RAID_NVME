"""
Smoke 测试 —— 随机 IO 压力 + 数据一致性（512 / 历史池）。
...
布局由 random_io_plan_512 生成；无 Input_Config CSV。
"""
import os
import time
...
from test_items.random_io_plan_512 import (
...
def test_random_io():
    item = (os.environ.get("RAID_NVME_ITEM") or "test_vd_io_06_random_io_512").strip() or "test_vd_io_06_random_io_512"
    ...
    allure.dynamic.title("FIO 测试: random_io_512")
    round_idx = 0
    ...
    while time.monotonic() < deadline:
        ...
        header = (
            f"{table}\n"
            f"[RANDOM_IO round {round_idx}] align=512 item={item} "
        ...
        allure.dynamic.description(
            f"Round {round_idx}: align=512 16 windows "
    ...
    print(
        ...
        f"budget={duration_seconds}s elapsed≈{elapsed}s align=512 "
