...
def test_reboot_powercycle():
    item = (os.environ.get('RAID_NVME_ITEM') or 'reboot_512').strip() or 'reboot_512'
    ...
    fio_args = build_fio_args("reboot", item, extra=["-l", str(loops)])
    ...
    with allure.step(f"异步触发 FIO 指令: {cmd_str}"):
        ...
        trigger_background_fio(stress_dir, "reboot", fio_args)
