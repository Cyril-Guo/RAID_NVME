...
远程测试因无进展被 watchdog 终止时，控制端先记录失败，再采集现场并回收报告。
即使前面的用例已经通过，也会为尚未生成 JUnit 的活动用例补一条异常结果；
已有失败结果不会重复补报，原生 pytest Allure 结果也不会再被 JUnit 转换重复展示。
例如两项通过、第三项超时，应统计为 `Total=3, passed=2, errors=1`，飞书列出异常用例名。
手动 abort 不补成执行失败，也不发送飞书卡片；此前已经发生的真实测试失败不会被抹去。

每个 Allure 结果中并列展示：

- **终端输出**：发布报告时抓取的完整 Jenkins Console 快照，不是单条异常或某个用例的 stdout。
- **测试结果**：本用例状态、报错与堆栈；超时补报包含用例、阶段、退出码和未返回命令。
- **日志收集**：节点执行日志、`debug_<ip>.log`、用例 FIO/命令日志及 monitor/debug 包。
  采集不到或 SCP 未复制完整的附件会明确列出缺失原因，不保留无提示的坏链接。

Console 中可搜索 `[ITEM_START]`、`[PHASE]`、`[CMD_START]`、`[CMD_END]`、`[FAILURE_CONTEXT]`。
只有开始、没有结束的命令是待检查卡点；`[CMD_END] rc=0` 只证明该命令已完成，不能认定它导致 hang。
日志采集阶段卡住会报告为执行/采集异常，不把已经通过的测试改判失败。

现场探测只读取进程状态、等待点、内核消息和可用的进程栈，不格式化磁盘、不触发重启或 SysRq。
现场 SSH 限时 45 秒，远端报告回收限时 180 秒，Allure SCP 限时 180 秒，汇总 JUnit SCP 限时 60 秒；
超时后有 5 秒强制退出宽限。备用单用例 JUnit SCP 每项限时 10 秒。
用例 debug 包最多收集 256 个文件、64 MiB 原始内容，单文件最多保留末尾 8 MiB；
截断、缺失与大小记录在包内 `collection_manifest.txt`。

目标机完全失联时只能保留控制端已有输出及连接/采集失败证据；无法凭空获取内核栈，
也不能仅凭 watchdog 超时确认驱动或硬件根因。旧构建不会因更新脚本而自动补齐丢失的现场。
